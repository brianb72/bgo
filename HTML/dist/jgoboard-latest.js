...
function process(JGO, div) {
  ...
  if(div.getAttribute('data-game-blurb')) {
    var blurb = div.getAttribute('data-game-blurb');
    jsetup.setOptions({blurb: blurb})
  }
...
var Canvas = function(elem, opt, images) {
  ...
  for(i=0; i<opt.view.height; i++) {
    if(opt.coordinates && opt.coordinates.left)
      ...
      this.ctx.fillText(C.COORDINATES[i + opt.view.yOffset],
          this.marginLeft / 2,
  ...
  this.ctx.font = opt.title_font.font;
  // Draw the blurb
  if(opt.blurb)
    this.ctx.fillText(opt.blurb,
          this.gridLeft + opt.grid.x * 9,
          this.marginTop / 2);
